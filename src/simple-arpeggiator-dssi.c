...
/* simple_arpeggiator.c

   DSSI Arpeggiator based on LMMS arpeggiator
   Constructed by Johan Berntsson
        ...
        snd_seq_event_t *events, unsigned long event_count)
{
    ...
    if (event_count > 0) {
        printf("simple_arp: have %ld events\n", event_count);
    }
    ...
    for (pos = 0, event_pos = 0; pos < sample_count; pos++) {
        ...
        while (event_pos < event_count
            ...
            printf("simple_arp: event type %d\n", events[event_pos].type);
...
#endif
{
    ...
    if (tsLDescriptor) {
        ...
        tsLDescriptor->Name = "Simple Arp";
        tsLDescriptor->Maker = "Johan Berntsson <johan@microheaven.com>";
